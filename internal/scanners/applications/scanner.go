...
// ApplicationScanner scans for installed applications
type ApplicationScanner struct{}

func (s *ApplicationScanner) Name() string {
	return "applications"
}

func (s *ApplicationScanner) Scan(cfg *config.Config) ([]scanners.Component, error) {
	if cfg.IsScannerDisabled("applications") {
		return nil, nil
	}

	switch runtime.GOOS {
	case "darwin":
		return scanDarwinApplications(cfg)
	case "windows":
		return scanWindowsApplications(cfg)
	case "linux":
		return scanLinuxApplications(cfg)
	default:
